...
data_pc_raw <- pdfetch_FRED(c("NGDPRSAXDCCAQ", "LRUN64TTCAM156S", "CANCPIALLMINMEI", "CANCPICORMINMEI"))
#Date adjustment
...
data_pc$unemployment_rate <- (data_pc$LRUN64TTCAM156S)# seasonally adjusted
...
data1$un_pi_gap <- data1$unemployment_rate + data1$infgap/(0.1839 * 100)
#Note that 0.1839 was the estimated coefficient of unemployment rate in model 3.
...
data5 <- na.omit(data4)
plot.xts(data5$unemployment_rate, col = "black", lwd = 3 , main = "Canada NAIRU", main.timespan = FALSE, lty = 3, ylim = c(2, 14))
