...
data_pc_raw <- pdfetch_FRED(c("NGDPRSAXDCITQ", "LRUN64TTITQ156S", "ITACPIALLMINMEI", "ITACPICORMINMEI"))
#Date adjustment
...
data_pc$unemployment_rate <- (data_pc$LRUN64TTITQ156S)# seasonally adjusted
...
data1$un_pi_gap <- data1$unemployment_rate + data1$infgap/(0.02738*100)
...
data5 <- na.omit(data4)
plot.xts(data5$unemployment_rate, col = "black", lwd = 3, main = "Italy NAIRU", main.timespan = FALSE, lty = 3, ylim = c(2, 14)) #unemployment rate
