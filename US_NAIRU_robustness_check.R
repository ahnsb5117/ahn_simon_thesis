...
data_pc_raw <- pdfetch_FRED(c("GDPC1", "UNRATE", "CPIAUCSL", "CPILFESL", "NROU")) #API fetching from FRED
...
data_pc$nc_unemployment_rate <- (data_pc$NROU) # non-cyclical unemployment rate
...
addLegend("topleft", on=1, 
          legend.names = c("Unemployment Rate", "NAIRU", "Natural Rate of Unemployment"), 
          lty=c(3, 1,4), lwd=c(3, 1,1),
